...
class PixelDiTModelConfig:
    ...
    precomputed_caption_embeddings_path: str | None = None
...
_LOCAL_GEMMA_2_2B_IT = "weights/pid/gemma-2-2b-it"
...
_TEXT_ENCODER_DICT = {
    ...
    "gemma-2-2b-it": _LOCAL_GEMMA_2_2B_IT if os.path.isdir(_LOCAL_GEMMA_2_2B_IT) else "Efficient-Large-Model/gemma-2-2b-it",
...
class PixelDiTModel(ImaginaireModel):
    ...
    def __init__(self, config: PixelDiTModelConfig):
        ...
        if config.precomputed_caption_embeddings_path:
            ...
            logger.info(
                "PixelDiTModel: using precomputed caption embeddings from "
                f"{config.precomputed_caption_embeddings_path}; Gemma text encoder is not loaded."
            )
        else:
            ...
            with misc.timer("PixelDiTModel: load_text_encoder"):
                ...
                self._null_caption_embs = self._encode_text_raw(
    ...
    def _encode_text_raw(self, captions: list[str]) -> tuple[Tensor, Tensor]:
        if self.text_encoder is None or self.tokenizer is None:
            raise RuntimeError(
                "PiD text encoder is disabled because precomputed caption embeddings are configured. "
                "Pass data_batch['caption_embs'] or unset precomputed_caption_embeddings_path."
            )
